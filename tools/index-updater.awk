...
END {
	...
	if (data_pri != primary) {
	    ...
	    text = data_pri;
	    # Here we have a bug branch - if a new pri starts as with pri+sec,
	    # we now ignore the sec in text but print its id. Instead, we might
	    # need to print a pri by itself followed by a 2nd entry at level=1,
	    # which is hard to do since the loop iterates in one direction only.
	    if (data_sec) {
		insert_pri = 1;
	    }
	} else {
	...
	if (insert_pri) {  # After closing all ul, we might need to indent more.
	    printf("  <li>%s</li>\n", text);
	    level_new = 1;
	    text = data_sec;  # insert_pri is true  if both data_pri and data_sec
	}
    ...
    print ""
    print "<!-- Index generated by tools/index-updater.awk, has " entries " references. -->";
    print ""
    print "</body>";
    print "</html>";
}
